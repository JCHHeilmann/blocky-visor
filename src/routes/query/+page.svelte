<script lang="ts">
	...
	let currentResult = $state<{ domain: string; type: string; response: DnsQueryResponse } | null>(null);
	...
	async function handleQuery(domain: string, type: string) {
		...
		try {
			...
			history = [{ domain, type, response, timestamp: Date.now() }, ...history].slice(0, MAX_HISTORY);
			saveHistory();
		} catch (err) {
			toastStore.error(`Query failed: ${err instanceof Error ? err.message : 'Unknown error'}`);
		} finally {
			loading = false;
		}
	}
...
<div class="space-y-6">
	...
	<div>
		<div class="mb-3 flex items-center justify-between">
			...
			{#if history.length > 0}
				<button onclick={clearHistory} class="text-xs text-text-muted hover:text-text-primary cursor-pointer">
					Clear
				</button>
			{/if}
		</div>
		...
		{:else}
			<div class="space-y-2">
				{#each history as entry}
					...
					>
						<span class="font-mono text-sm text-accent-600 dark:text-accent-400">{entry.domain}</span>
						<span class="rounded bg-surface-secondary px-2 py-0.5 text-xs text-text-secondary">{entry.type}</span>
						<span class="rounded px-2 py-0.5 text-xs
							{entry.response.returnCode === 'NOERROR' ? 'bg-green-500/10 text-green-600 dark:text-green-400' : 'bg-red-500/10 text-red-600 dark:text-red-400'}">
							{entry.response.returnCode}
						</span>
						<span class="ml-auto text-xs text-text-faint">{formatRelativeTime(entry.timestamp)}</span>
					</button>
				{/each}
			</div>
		{/if}
