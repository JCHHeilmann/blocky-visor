<script lang="ts">
    ...
    import { fetchStats, fetchTimeline, type StatsRange } from "$lib/api/sidecar-stats";
    import { formatNumber, formatPercentage } from "$lib/utils/formatters";
    import type { ParsedMetrics, SidecarStatsResponse, SidecarTimelineBucket } from "$lib/types/api";
    ...
    let cacheHitRate = $derived(
        metrics?.cacheHits !== undefined && metrics?.cacheMisses !== undefined
            ? formatPercentage(metrics.cacheHits, metrics.cacheHits + metrics.cacheMisses)
            : undefined,
    );

    let avgLatency = $derived.by(() => {
        if (!metrics?.requestDurationSum || !metrics?.requestDurationCount) return undefined;
        const avgMs = (metrics.requestDurationSum / metrics.requestDurationCount) * 1000;
    ...
    let chartTitle = $derived(
        range === '7d' ? 'Daily Breakdown (7 Days)' :
        range === '30d' ? 'Daily Breakdown (30 Days)' :
        range === 'yesterday' ? 'Hourly Breakdown (Yesterday)' :
    ...
    let statsTotal = $derived(
        isHistorical
            ? (sidecarStats ? formatNumber(sidecarStats.summary.total_queries) : 'N/A')
            : formatNumber(metrics?.totalQueries)
    );
    let statsTotalTrend = $derived(
        isHistorical
            ? (sidecarStats ? `${sidecarStats.summary.unique_domains.toLocaleString()} unique domains` : undefined)
    ...
    let statsBlocked = $derived(
        isHistorical
            ? (sidecarStats ? formatNumber(sidecarStats.summary.blocked_queries) : 'N/A')
            : formatNumber(blockedCount)
    );
    let statsBlockedTrend = $derived.by(() => {
        if (isHistorical) {
            if (!sidecarStats) return undefined;
            const pct = sidecarStats.summary.total_queries > 0
                ? ((sidecarStats.summary.blocked_queries / sidecarStats.summary.total_queries) * 100).toFixed(1)
                : '0';
            return `${pct}% of total`;
        }
    ...
    let statsCard3Label = $derived(isHistorical ? 'Cached Queries' : 'Cache Hit Rate');
    let statsCard3Value = $derived(
        isHistorical
            ? (sidecarStats ? formatNumber(sidecarStats.summary.cached_queries) : 'N/A')
            : cacheHitRate ?? 'N/A'
    );
    let statsCard3Trend = $derived.by(() => {
        if (isHistorical) {
            if (!sidecarStats) return undefined;
            const pct = sidecarStats.summary.total_queries > 0
                ? ((sidecarStats.summary.cached_queries / sidecarStats.summary.total_queries) * 100).toFixed(1)
                : '0';
    ...
    let statsLatency = $derived(
        isHistorical
            ? (sidecarStats ? `${sidecarStats.summary.avg_duration_ms.toFixed(1)}ms` : 'N/A')
            : avgLatency ?? 'N/A'
    );
    let statsLatencyTrend = $derived(
        isHistorical
            ? (sidecarStats ? `P95: ${sidecarStats.summary.p95_duration_ms}ms` : undefined)
...
<div class="space-y-6">
    ...
    <Card>
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="text-lg font-semibold text-text-primary mb-1">DNS Blocking</h2>
    ...
    {:else}
        <Card>
            ...
            {:else if sidecarStats}
                <div class="relative">
                    {#if sidecarLoading}
                        <div class="absolute inset-0 z-10 flex items-center justify-center rounded-lg bg-surface-primary/60">
                            <div class="h-5 w-5 animate-spin rounded-full border-2 border-accent-600 border-t-transparent"></div>
                        </div>
                    {/if}
    ...
    <div class="grid gap-4 lg:grid-cols-2">
        {#if mode === 'live'}
            <Card>
                <h3 class="text-sm font-medium text-text-secondary mb-3">Response Breakdown</h3>
            ...
            <Card>
                <h3 class="text-sm font-medium text-text-secondary mb-3">Queries by Client</h3>
                <div class="min-h-[200px]">
                    <ClientTable
        ...
        {:else}
            <Card>
                ...
                <div class="min-h-[200px]">
                    ...
                    {:else if sidecarStats}
                        <div class="relative">
                            {#if sidecarLoading}<div class="absolute inset-0 z-10 rounded-lg bg-surface-primary/60"></div>{/if}
                            <ResponseBreakdownChart categories={sidecarStats.response_categories} />
                        </div>
                    {/if}
                </div>
            </Card>
    ...
    {#if mode === 'live'}
        <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <StatsCard
                ...
                trend={metrics?.failedDownloads !== undefined && metrics.failedDownloads > 0
        ...
        <div class="grid gap-4 lg:grid-cols-2">
            ...
            <Card>
                ...
                {:else if sidecarStats}
                    <div class="relative">
                        {#if sidecarLoading}<div class="absolute inset-0 z-10 rounded-lg bg-surface-primary/60"></div>{/if}
                        <DistributionChart data={sidecarStats.return_codes} colorFn={returnCodeColor} maxItems={5} />
                    </div>
                {/if}
            </Card>
        </div>
