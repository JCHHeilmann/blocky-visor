...
<div class="max-w-2xl space-y-6">
  <Card>
    <h2 class="mb-4 text-lg font-semibold text-text-primary">API Connection</h2>
    <ApiUrlForm />
  </Card>
  ...
  {#if blockyInfo?.buildInfo || blockyInfo?.lastListRefresh !== undefined}
    <Card>
      ...
      <div class="space-y-3 text-sm">
        ...
        {#if blockyInfo.lastListRefresh}
          <div class="flex justify-between">
            ...
            <span class="font-mono text-text-primary"
              >{formatDate(blockyInfo.lastListRefresh * 1000)}</span
            >
          </div>
        {/if}
      ...
      <p class="mt-4 text-xs text-text-faint">
        Blocky configuration is managed via YAML config files. Changes require
        editing the config file and restarting the Blocky service.
      </p>
    </Card>
  {/if}
