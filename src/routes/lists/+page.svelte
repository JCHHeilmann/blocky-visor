<script lang="ts">
	...
	async function handleRefresh() {
		...
		try {
			...
			toastStore.error(`Failed to refresh lists: ${err instanceof Error ? err.message : 'Unknown error'}`);
		} finally {
			loading = false;
		}
	}
...
<div class="space-y-6">
	<Card>
		<div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
			<div>
				...
				{#if lastRefresh}
					<p class="mt-1 text-xs text-text-faint">Last refreshed: {formatDate(lastRefresh)}</p>
				{/if}
			</div>
	...
	{#if metrics?.listEntries && Object.keys(metrics.listEntries).length > 0}
		<Card>
			<h3 class="mb-4 text-sm font-medium text-text-secondary">List Entry Counts</h3>
			<div class="space-y-3">
				{#each Object.entries(metrics.listEntries) as [name, count]}
					<div class="flex items-center justify-between">
						<span class="truncate font-mono text-sm text-text-secondary">{name}</span>
						<span class="ml-4 shrink-0 text-sm font-medium text-text-primary">{formatNumber(count)}</span>
					</div>
				{/each}
			</div>
		</Card>
	{:else}
		<Card>
			<p class="text-center text-sm text-text-muted">
				{metrics ? 'No list entries found in metrics' : 'Prometheus metrics unavailable — enable metrics in Blocky for entry counts'}
			</p>
		</Card>
	{/if}
</div>

<Modal open={showConfirm} onclose={() => (showConfirm = false)} title="Refresh Lists">
	<p class="text-sm text-text-secondary">
		This will re-download and reload all configured blocking lists. This may take a moment.
	</p>
	{#snippet actions()}
