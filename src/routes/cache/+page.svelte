<script lang="ts">
	...
	async function handleFlush() {
		...
		try {
			...
			toastStore.error(`Failed to flush cache: ${err instanceof Error ? err.message : 'Unknown error'}`);
		} finally {
			loading = false;
		}
	}
...
<div class="space-y-6">
	...
	<div class="grid gap-4 sm:grid-cols-3">
		...
		<StatsCard
			...
			trend={metrics?.cacheHits !== undefined && metrics?.cacheMisses !== undefined
	...
	{#if !metrics}
		<Card>
			<p class="text-center text-sm text-text-muted">
				Prometheus metrics unavailable — enable metrics in Blocky for cache statistics
			</p>
		</Card>
	{/if}
</div>

<Modal open={showConfirm} onclose={() => (showConfirm = false)} title="Flush Cache">
	<p class="text-sm text-text-secondary">
		This will clear all cached DNS responses. Subsequent queries will be resolved fresh from upstream servers.
	</p>
	{#snippet actions()}
