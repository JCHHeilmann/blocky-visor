...
export async function sidecarRequest<T>(
	...
	try {
		...
		if (!response.ok) {
			...
			throw new ApiError(`Sidecar error: ${response.status}`, response.status, body);
		}
...
export async function testSidecarConnection(baseUrl: string, apiKey: string): Promise<boolean> {
	try {
		const controller = new AbortController();
