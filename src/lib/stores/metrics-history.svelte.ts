...
function createMetricsHistoryStore() {
	...
	function push(metrics: ParsedMetrics) {
		snapshots = [...snapshots, { timestamp: Date.now(), metrics }].slice(-MAX_SNAPSHOTS);
	}
	...
	return {
		...
		get blockedPerInterval(): number | undefined {
			...
			const prev = snapshots[snapshots.length - 2].metrics;
			const curr = snapshots[snapshots.length - 1].metrics;
			const delta = sumBlockedResponses(curr.responsesByReason) - sumBlockedResponses(prev.responsesByReason);
			return delta >= 0 ? delta : undefined;
		},
...
function sumBlockedResponses(responsesByReason?: Record<string, number>): number {
