<script lang="ts">
	...
	let barWidth = $derived(
		data.length > 0 ? Math.max(4, (chartWidth - (data.length - 1) * BAR_GAP) / data.length) : 20
	);
...
<div bind:this={containerEl} class="w-full">
	{#if data.length === 0}
		<div class="flex h-[120px] items-center justify-center text-sm text-text-muted">
	...
	{:else}
		...
		<div class="mt-2 flex items-center gap-4 text-xs text-text-muted">
			<span class="flex items-center gap-1.5">
				<span class="inline-block h-2.5 w-2.5 rounded-sm bg-accent-500/70"></span>
				Allowed
			</span>
			<span class="flex items-center gap-1.5">
