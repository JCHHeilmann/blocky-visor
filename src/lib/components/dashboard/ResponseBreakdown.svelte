<script lang="ts">
	...
	let { responsesByReason, total }: Props = $props();
	...
	const REASON_COLORS: Record<string, { bar: string; dot: string; hex: string }> = {
		BLOCKED: { bar: 'bg-red-500/70 dark:bg-red-500/60', dot: 'bg-red-500', hex: '#ef4444' },
		CACHED: { bar: 'bg-accent-500/70 dark:bg-accent-500/60', dot: 'bg-accent-500', hex: '#14b8a6' },
		RESOLVED: { bar: 'bg-emerald-500/70 dark:bg-emerald-500/60', dot: 'bg-emerald-500', hex: '#10b981' },
		CONDITIONAL: { bar: 'bg-amber-500/70 dark:bg-amber-500/60', dot: 'bg-amber-500', hex: '#f59e0b' },
		CUSTOM: { bar: 'bg-violet-500/70 dark:bg-violet-500/60', dot: 'bg-violet-500', hex: '#8b5cf6' },
		SPECIAL: { bar: 'bg-sky-500/70 dark:bg-sky-500/60', dot: 'bg-sky-500', hex: '#0ea5e9' }
	};

	const FALLBACK_COLOR = { bar: 'bg-gray-500/70 dark:bg-gray-500/60', dot: 'bg-gray-500', hex: '#6b7280' };
	...
	let segments = $derived.by(() => {
		if (!responsesByReason || !total || total === 0) return [];
		return Object.entries(responsesByReason)
			.sort(([, a], [, b]) => b - a)
...
{:else}
	<!-- Stacked bar -->
	<div class="flex h-4 w-full overflow-hidden rounded-full bg-surface-secondary">
	...
	<div class="mt-4 space-y-2">
		{#each segments as seg}
			<div class="flex items-center gap-3 text-sm">
				...
				<span class="shrink-0 tabular-nums font-mono text-text-secondary">{formatNumber(seg.count)}</span>
				<span class="shrink-0 w-14 text-right tabular-nums font-mono text-text-muted">{seg.pct}</span>
			</div>
		{/each}
	</div>
{/if}
