...
<div class="flex items-center gap-4">
  ...
  <div>
    <p
      class="font-medium {blockingStore.enabled
...
>
  <p class="mb-4 text-sm text-text-secondary">
    How long should blocking be disabled?
  </p>
  ...
  {#if selectedDuration === "custom"}
    <div class="mt-3">
      <label for="custom-minutes" class="block text-sm text-text-secondary mb-1"
        >Minutes</label
      >
      <input
        id="custom-minutes"
  ...
  {#snippet actions()}
    <Button variant="ghost" onclick={() => (showDisableModal = false)}
      >Cancel</Button
    >
