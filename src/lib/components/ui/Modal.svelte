...
{#if open}
  ...
  >
    <div
      class="w-full max-w-md rounded-xl border border-surface-border bg-surface-primary shadow-[var(--shadow-overlay)] animate-fade-in-scale"
    >
      <div
      ...
      {#if actions}
        <div
          class="flex justify-end gap-3 border-t border-surface-border px-5 py-4"
        >
          {@render actions()}
        </div>
      {/if}
