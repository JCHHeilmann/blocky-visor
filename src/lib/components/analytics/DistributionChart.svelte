<script lang="ts">
	...
	let displaySorted = $derived.by(() => {
		...
		const restCount = sorted.slice(maxItems).reduce((sum, [, count]) => sum + count, 0);
		if (restCount > 0) {
	...
	const defaultHexColors = [
		'#0d9488', '#3b82f6', '#a855f7', '#f59e0b',
		'#10b981', '#f43f5e', '#06b6d4', '#f97316'
	];
	...
	function showBarTooltip(e: MouseEvent, key: string, count: number, index: number) {
			...
			<div>
				<div class="flex items-center justify-between mb-1">
					...
					<span class="text-sm tabular-nums">
						<span class="font-mono text-text-secondary">{formatNumber(count)}</span>
