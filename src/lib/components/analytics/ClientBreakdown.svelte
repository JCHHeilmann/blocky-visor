<script lang="ts">
	...
	function showClientTooltip(e: MouseEvent, client: ClientData) {
		...
		const pct = grandTotal > 0 ? ((client.total / grandTotal) * 100).toFixed(1) : '0';
		const html = `
			...
			<div class="mt-1 text-xs space-y-0.5">
				...
				${client.blocked > 0 ? `<div class="flex justify-between gap-4">
				...
				<div class="flex items-center justify-between mb-1">
					<span class="truncate text-sm min-w-0">
						{#if hasHostname(client)}
							...
							<span class="text-text-faint ml-1.5 font-mono text-xs">{client.ip}</span>
						{:else}
							<span class="text-text-secondary font-mono">{client.ip}</span>
						{/if}
					</span>
					<span class="shrink-0 text-sm font-mono text-text-primary ml-2">{formatNumber(client.total)}</span>
				</div>
				<div class="h-1.5 w-full overflow-hidden rounded-full bg-surface-secondary">
					<div class="flex h-full">
						<div class="h-full rounded-l-full bg-accent-500/70 transition-all" style="width: {totalPct - blockedPct}%"></div>
						{#if blockedPct > 0}
							<div class="h-full rounded-r-full bg-red-500/60 transition-all" style="width: {blockedPct}%"></div>
						{/if}
					</div>
				</div>
			</div>
