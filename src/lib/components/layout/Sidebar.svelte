<script lang="ts">
  ...
  let links = $derived([
    ...baseLinks,
    ...(sidecarStore.configured ? sidecarLinks : []),
    {
      href: "/settings",
      label: "Settings",
...
<nav
  class="flex h-full w-full flex-col bg-surface-primary border-r border-surface-border"
>
  <!-- Logo — h-14 matches the header height -->
  <div
    class="flex h-14 shrink-0 items-center gap-3 border-b border-surface-border px-4"
  >
    <div
      class="flex h-8 w-8 items-center justify-center rounded-lg bg-accent-600"
    >
      <svg
    ...
    {#if !collapsed}
      <span class="text-lg font-bold text-text-primary">Blocky Visor</span>
    {/if}
  </div>
        ...
        <li>
          <a
            ...
            class="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-all duration-150
							...
							{isActive(link.href)
              ? 'bg-accent-600/15 text-accent-600 dark:text-accent-400 border-l-2 border-accent-600 dark:border-accent-400'
  ...
  <div class="border-t border-surface-border px-3 py-3">
    <div class={collapsed ? "flex justify-center" : "px-3"}>
      <StatusBadge
        status={blockingStore.connected ? "online" : "offline"}
        label={collapsed
          ? undefined
          : blockingStore.connected
            ? "Connected"
            : "Disconnected"}
      />
    </div>
