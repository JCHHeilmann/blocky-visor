<script lang="ts">
	...
	async function loadStatus() {
		...
		try {
			...
			error = err instanceof Error ? err.message : 'Failed to get service status';
		} finally {
			loading = false;
		}
	}
...
<div class="space-y-4">
		...
		<div class="space-y-3 text-sm">
			<div class="flex items-center justify-between">
				...
				<span class="flex items-center gap-2">
					<span class="h-2 w-2 rounded-full {isActive ? 'bg-green-500' : 'bg-red-500'}"></span>
					<span class="font-medium {isActive ? 'text-green-400' : 'text-red-400'}">
	...
	<div class="flex gap-3">
		<Button variant="danger" onclick={() => (showConfirm = true)} loading={restarting}>
			Restart Blocky
		</Button>
...
<Modal open={showConfirm} onclose={() => (showConfirm = false)} title="Restart Blocky">
	{#snippet children()}
		<p class="text-sm text-text-secondary">
			This will restart the Blocky DNS service. DNS resolution will be briefly interrupted during the restart.
		</p>
	{/snippet}
	{#snippet actions()}
